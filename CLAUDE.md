...
`HERO_QUOTES` is a plain array (`{ quoteLines, attribution, bust, bustAlt }`) — add entries as more busts get made (Pete Rock, Bob Dylan, Linda Ronstadt as of 2026-07-24). On mount a random entry is picked (`Math.floor(Math.random() * HERO_QUOTES.length)`); a `setInterval` (`ROTATE_MS`, 16s) then live-cycles through the set with a `FADE_MS` (400ms) opacity crossfade while the page stays open — a deliberate choice over reload-only rotation, to match the site's "always-on broadcast" feel; the `HERO_QUOTES.length < 2` early-return only matters if the array is ever back down to a single entry. Clicking the headline text (not the bust — that's the tilt/drag target) manually calls the same advance-with-crossfade function and restarts the rotation timer from that point, so a click isn't immediately followed by another auto-advance a few seconds later.

**Keep each `quoteLines` line to ~20 characters or less.** A line longer than that (Bob Dylan's original "from Cézanne as I have" at 23 chars) can overflow the column width and soft-wrap *inside* that one hand-set line — e.g. it wrapped as "FROM CÉZANNE AS I" / "HAVE" with "HAVE" stranded alone, which reads badly (reported via screenshot, fixed 2026-07-24 by re-splitting into more/shorter lines). The line-break mechanism itself (no `<br/>` before the last line, so the attribution shares its baseline) is unaffected by this — it's purely about each individual line fitting on its own without an unwanted internal wrap.

`quoteLines` is an array of hand-set line strings, not one long string relying on CSS to wrap it — the attribution is appended directly after the last line inside the same `<p>` (no `<br/>` before it), so it shares that line's text baseline for free via normal inline layout instead of needing manual position tweaks. This was a deliberate fix (2026-07-24): relying on natural text-wrap to land the attribution "at the end of the last line" is NOT reliable — depending on where the browser happens to break the paragraph, the attribution can get stranded wrapping onto its own line below (tried both plain inline and `float-right`; both still wrap away if the actual last rendered line doesn't have room). Hardcoding the line breaks (mirroring how the original mockup itself was hand-composed, not fluidly wrapped) guarantees the last line always has the attribution glued to it. If you add a new quote, pick line breaks short enough to leave trailing room for the attribution on that last line, same as this one does.

The `<section>` root carries a `-mt-[14px]` — that's a deliberate 20% reduction (2026-07-24) of the page's global `gap-[70px]` flex-column spacing (see `NebulaHomepage.tsx`'s root div) specifically between the header and this section, not a stray leftover. `NebulaHomepage`'s gap is one uniform value shared by every top-level section, so pulling in just this one gap requires a negative margin on this component rather than touching the shared value (which would shrink spacing everywhere).
