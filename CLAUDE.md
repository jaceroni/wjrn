...
- **The mobile tree has no "WEEKLY LIVE BROADCAST SCHEDULE" list** (removed 2026-07-24) — the title + description + full event list together overflowed `MOBILE_CONTENT_ZONE` and ran up over the video screen window above it. The desktop tree still has the full schedule list; only the mobile JSX return dropped it. If mobile ever gets more vertical room to work with (a taller graphic, etc.), reintroducing it would mean re-adding the `Calendar` icon + label + `BROADCAST_EVENTS.map(...)` block that used to sit right after the description paragraph in that branch — check git history for the exact markup rather than re-deriving it from the desktop version, since the desktop version uses different sizing (`text-[10px] xl:text-xs` vs. the mobile tree's `text-[9px]`).
...
Sits between the header and the vintage receiver embed on the homepage. Big chunky `font-display` quote (gold `“`/`”` bookend glyphs via `#d7b158`, cream body via `#faf6f0`, matching the site's established off-white — see `TwitchScheduleRetro.tsx`'s `#faf6f0` headings) with a small white attribution below it (`text-[17px] sm:text-[22px]` — the mobile size is a deliberate 25%-off `22px` per explicit request 2026-07-24, not an oversight), and a sculpted bust image to the right using the **exact same ambient-tilt + click-drag interaction as the About page team busts** (`AboutWjrn.tsx`) — look-at-cursor `atan2` trig, drag overrides ambient until release — just a single bust instead of a per-index map since only one is ever shown at a time.
...
Below `md`, the gap between the header divider and each page's first content section is tuned to the same **52px** on `NebulaHomepage`, `StationLanding`, and `AboutWjrn` (2026-07-24, explicit request that all three match). Each page gets there differently because each had a different starting mechanism, so don't assume one shared class will work everywhere:
- `NebulaHomepage`: `HeroQuote`'s root `-mt-[18px] md:-mt-[14px]` against the page's `gap-[70px]` (70 − 18 = 52 on mobile; 70 − 14 = 56 unchanged on desktop).
- `StationLanding`: the hero grid section's `mt-[52px] md:mt-15` — this page never used the `gap-[70px]` pattern, it's a direct `margin-top` (`mt-15` = 60px was the pre-existing desktop value).
- `AboutWjrn`: the Hero section's `-mt-[18px] md:mt-0` against its own `gap-[70px]` (70 − 18 = 52 on mobile; `md:mt-0` cancels the offset back to the untouched 70px desktop gap).
