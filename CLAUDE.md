...
`HERO_QUOTES` is a plain array (`{ quoteLines, attribution, bust, bustAlt }`) — add entries as more busts get made. On mount a random entry is picked (`Math.floor(Math.random() * HERO_QUOTES.length)`); a `setInterval` (`ROTATE_MS`, 16s) then advances through the set with a `FADE_MS` (400ms) opacity crossfade. With only one entry today the rotation effect is a no-op by design (`HERO_QUOTES.length < 2` early-returns) rather than dead code — it's already wired for whenever a second quote/bust is added, no rewiring needed.
