...
`HERO_QUOTES` is a plain array (`{ quoteLines, attribution, bust, bustAlt }`) — add entries as more busts get made (Pete Rock + Bob Dylan as of 2026-07-24). On mount a random entry is picked (`Math.floor(Math.random() * HERO_QUOTES.length)`); a `setInterval` (`ROTATE_MS`, 16s) then live-cycles through the set with a `FADE_MS` (400ms) opacity crossfade while the page stays open — a deliberate choice over reload-only rotation, to match the site's "always-on broadcast" feel; the `HERO_QUOTES.length < 2` early-return only matters if the array is ever back down to a single entry.
