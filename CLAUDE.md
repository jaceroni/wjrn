...
**Mobile sizing has gone through two rounds of cuts, both deliberate, not oversights**: quote text is `text-[16px] sm:text-5xl md:text-6xl lg:text-[72px]`, attribution is `text-[9px] sm:text-[22px]` — 2026-07-24 first took the attribution down 25% (22→17), then a follow-up request cut both the quote (32→16) and the attribution (17→9) by 50% more, specifically on mobile only (`sm:` and up are untouched both times). The attribution is also `text-center sm:text-right` — centered on mobile to match the quote block's own `text-center lg:text-left`, right-aligned again from `sm` up to match the original mockup.
