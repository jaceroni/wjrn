...
- `isLiveActive` comes from the real Twitch Embed SDK `ONLINE`/`OFFLINE` player events (`embed.getPlayer().addEventListener(...)`), not a schedule-based guess. **`embed.getPlayer()` can momentarily return an object that isn't fully initialized right as `VIDEO_READY` fires** — calling `.addEventListener` on it then throws (console shows `"x.addEventListener is not a function"`, thrown from inside Twitch's `v1.js` while forwarding the event to our handler). That exception silently killed ONLINE/OFFLINE registration entirely, so `isLiveActive` never updated and the card stayed stuck on the countdown even while the channel was genuinely live (fixed 2026-07-25) — the card looks identical to "not live" in both cases since the opaque glass graphic covers the video either way, so this bug was easy to mistake for the stream just not playing. The fix retries `getPlayer()` (bounded, ~25 attempts / 200ms apart) until it actually has `.addEventListener`, instead of assuming it's ready the instant `VIDEO_READY` fires.
- Both `#twitch_schedule_module` wrappers (desktop and mobile) carry an explicit `aspectRatio` inline style (`923/388` desktop, `582/657` mobile) matching the cabinet PNGs' native dimensions — added alongside the above fix because the console also logged a Twitch autoplay warning ("requirements for autoplay were not met: style visibility"), which points to the container having near-zero rendered height for a moment before the `<img>` (sized via `w-full h-auto`, no width/height attributes) finishes loading and establishes real height through normal layout. Reserving the aspect ratio up front means the video's absolutely-positioned container has real, non-zero size from first paint, regardless of image load timing.
